...
describe('formatBaselineAndAnomaly', () => {
  describe('rare detector', () => {
    ...
    it('falls back to "—" anomaly when anomalousValue is absent', () => {
      const { anomaly } = formatBaselineAndAnomaly(
        makeEntry({ detectorFunction: 'rare', anomalousValue: undefined, anomalousValueCount: 0 })
      );
      expect(anomaly).toBe('—');
    });
  });
  ...
  describe('metric detector (high_count, high_mean, etc.)', () => {
    ...
    it('returns "—" for anomaly when anomalousValue is absent', () => {
      const { anomaly } = formatBaselineAndAnomaly(
        makeEntry({ detectorFunction: 'high_count', fieldName: null, anomalousValue: undefined })
      );
      expect(anomaly).toBe('—');
    });
  });
