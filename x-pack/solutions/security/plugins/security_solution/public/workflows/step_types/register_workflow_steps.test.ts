...
import { updateAssetCriticalityStepDefinition } from './update_asset_criticality_step/update_asset_criticality_step';
...
describe('registerWorkflowSteps (public)', () => {
  ...
  it('async loader returns step definitions when feature flag is enabled', async () => {
    ...
    const [loader1, loader2, loader3, loader4, loader5, loader6, loader7, loader8, loader9] =
    ...
    await expect(loader6()).resolves.toBe(updateAssetCriticalityStepDefinition);
    await expect(loader7()).resolves.toBe(assignAttackStepDefinition);
    await expect(loader8()).resolves.toBe(setAttackStatusStepDefinition);
    await expect(loader9()).resolves.toBe(setAttackTagsStepDefinition);
  });
