...
import { UpdateAssetCriticalityStepId } from '../../../common/workflows/step_types/update_asset_criticality_step/update_asset_criticality_step_common';
...
describe('registerWorkflowSteps (server)', () => {
  ...
  it('async loader returns step definitions when feature flag is enabled', async () => {
    ...
    const [loader1, loader2, step3, step4, step5, step6, step7, step8, step9] =
    ...
    expect((step6 as ServerStepDefinition).id).toBe(UpdateAssetCriticalityStepId);
    expect(step7).toBe(assignAttackStepDefinition);
    expect(step8).toBe(setAttackStatusStepDefinition);
    expect(step9).toBe(setAttackTagsStepDefinition);
  });
