...
export const getAnomalyDetectionInlineTool = (
  ...
  handler: async ({ entityType, prompt, queryExtraContext }, context) => {
    try {
      ...
      const mlModulesProvider = ctx.ml?.modulesProvider(request, soClient);
      const modules = await mlModulesProvider?.listModules?.();

      const jobServiceProvider = ctx.ml?.jobServiceProvider(request, soClient);
      ...
      const formattedModulesJobs = modules?.map((module) => {
        return {
          ...
          moduleJobs: module.jobs.filter(isMlSecurityJob).map((job) => {
      ...
      const securityJobsFormatted =
        formattedModulesJobs?.flatMap((module) => module.moduleJobs) ?? [];

      const recommendedJobs = securityJobsFormatted.filter((job) =>
      ...
      if (recommendedStartedJobIds.length === 0) {
        message = getNoJobsMessage(entityType, recommendedJobIds, securityJobsFormatted);
      } else {
        const whereList = recommendedStartedJobIds.map((id) => `"${id}"`).join(', ');
        const whereClause = `| WHERE job_id IN (${whereList})`;
        message = getMessage(anomalyScore, whereClause, entityType, recommendedJobs);
      }
